import torch
import torch.nn as nn
from torch.nn.utils import weight_norm
from .base import MASRModel
import feature
import math

# https://arxiv.org/pdf/1807.06521.pdf
class AttentionBlock(nn.Module):
    def __init__(self, v):
        super().__init__()
        layer1 = nn.Linear(v, v // 2)
        nn.init.kaiming_normal_(layer1.weight)
        layer1 = weight_norm(layer1)
        layer2 = nn.Linear(v // 2, v)
        nn.init.kaiming_normal_(layer2.weight)
        layer2 = weight_norm(layer2)
        dropout = nn.Dropout(0.2, inplace=True)
        modules = []
        modules.append(layer1)
        modules.append(nn.ReLU())
        modules.append(layer2)
        modules.append(dropout)
        self.full = nn.Sequential(*modules)
        
        
    def forward(self, x):
        x = self.full(x)
        return x


# https://arxiv.org/pdf/2210.05265.pdf
class AttentionBlock1(nn.Module):
    def __init__(self, v):
        super().__init__()
        self.v = v
        ln1 = nn.Linear(v, v)
        nn.init.kaiming_normal_(ln1.weight)
        ln1 = weight_norm(ln1)
        self.layer_q = nn.Sequential(ln1, nn.Dropout(0.2, inplace=True))
        ln2 = nn.Linear(v, v)
        nn.init.kaiming_normal_(ln2.weight)
        ln2 = weight_norm(ln2)
        self.layer_k = nn.Sequential(ln2, nn.Dropout(0.2, inplace=True))
        ln3 = nn.Linear(v, v)
        nn.init.kaiming_normal_(ln3.weight)
        ln3 = weight_norm(ln3)
        self.layer_v = nn.Sequential(ln3, nn.Dropout(0.2, inplace=True))
        self.s = nn.Softmax(dim=1)

    def forward(self, x):
        x = x. transpose(1, 2)
        x = x.contiguous()
        q = self.layer_q(x)  
        k = self.layer_k(x)
        k = k.transpose(1, 2)
        k = k.contiguous()
        v = self.layer_v(x)
        v = v.transpose(1, 2)
        v = v.contiguous()
        a = torch.bmm(q, k) / math.sqrt(self.v)
        a_s = self.s(a)
        x = torch.bmm(v, a_s)
        return x




class ConvBlock(nn.Module):
    def __init__(self, conv, p):
        super().__init__()
        self.conv = conv
        nn.init.kaiming_normal_(self.conv.weight)
        self.conv = weight_norm(self.conv)
        self.act = nn.GLU(1)
        self.dropout = nn.Dropout(p, inplace=True)

    def forward(self, x):
        x = self.conv(x)
        x = self.act(x)
        x = self.dropout(x)
        return x


class GatedConv(MASRModel):
    """ This is a model between Wav2letter and Gated Convnets.
        The core block of this model is Gated Convolutional Network"""

    def __init__(self, vocabulary, blank=0, name="masr"):
        """ vocabulary : str : string of all labels such that vocaulary[0] == ctc_blank  """
        super().__init__(vocabulary=vocabulary, name=name, blank=blank)
        self.blank = blank
        self.vocabulary = vocabulary
        self.name = name
        output_units = len(vocabulary)
        self.attention = AttentionBlock(161)
        # self.attention1 = AttentionBlock1(161)
        self.norm = nn.LayerNorm(161)
        self.s = nn.Softmax(dim=0)
        modules = []
        modules.append(ConvBlock(nn.Conv1d(161, 500, 48, 2, 97), 0.2))

        for i in range(7):
            modules.append(ConvBlock(nn.Conv1d(250, 500, 7, 1), 0.3))

        modules.append(ConvBlock(nn.Conv1d(250, 2000, 32, 1), 0.5))

        modules.append(ConvBlock(nn.Conv1d(1000, 2000, 1, 1), 0.5))

        modules.append(weight_norm(nn.Conv1d(1000, output_units, 1, 1)))

        self.cnn = nn.Sequential(*modules)

    def forward(self, x, lens):  # -> B * V * T
        # attention type 1
        
        max_pool = nn.MaxPool3d((x.shape[0], 1, x.shape[2]), stride=1)
        avg_pool = nn.AvgPool3d((x.shape[0], 1, x.shape[2]), stride=1)
        mp = max_pool(x.unsqueeze(0))
        ap = avg_pool(x.unsqueeze(0))
        mp = self.attention(torch.squeeze(mp))
        ap = self.attention(torch.squeeze(ap))
        attn = mp + ap 
        attn = attn.unsqueeze(0)
        attn = self.norm(attn)
        attn = self.s(attn)
        attn = attn.unsqueeze(2)
        x = x * attn
        
        # attention type 2
        '''
        x = self.attention1(x)
        '''
        
        x = self.cnn(x)
        for module in self.modules():
            if type(module) == nn.modules.Conv1d:
                lens = (
                    lens - module.kernel_size[0] + 2 * module.padding[0]
                ) // module.stride[0] + 1
        return x, lens

    def predict(self, path):
        self.eval()
        wav = feature.load_audio(path)
        spec = feature.spectrogram(wav)
        spec.unsqueeze_(0)
        x_lens = spec.size(-1)
        out = self.cnn(spec)
        out_len = torch.tensor([out.size(-1)])
        text = self.decode(out, out_len)
        self.train()
        return text[0]
